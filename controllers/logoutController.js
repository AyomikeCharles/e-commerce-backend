...
const logout = asyncHandler( async (req, res) => {
    ...
    if(!OwnerOfToken){
        ...
        res.clearCookie('jwtRefreshToken', {httpOnly:true,  maxAge:24 * 60 * 60 * 1000})//change httpOnly to httpsOnly in production, also add secure:true and sameSite:'none' if project is added on different hosting platform
        res.status(200).json({message:"already logged out"})
        }
